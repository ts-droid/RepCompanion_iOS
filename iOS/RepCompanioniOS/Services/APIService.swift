...
class APIService {
    ...
    func updateUserProfile(
        goalStrength: Int? = nil,
        goalVolume: Int? = nil,
        goalEndurance: Int? = nil,
        goalCardio: Int? = nil,
        ...
        var body: [String: Any] = [:]
        if let goalStrength = goalStrength { body["goalStrength"] = goalStrength }
        if let goalVolume = goalVolume { body["goalVolume"] = goalVolume }
        if let goalEndurance = goalEndurance { body["goalEndurance"] = goalEndurance }
        if let goalCardio = goalCardio { body["goalCardio"] = goalCardio }
        if let sessionsPerWeek = sessionsPerWeek { body["sessionsPerWeek"] = sessionsPerWeek }
        if let sessionDuration = sessionDuration { body["sessionDuration"] = sessionDuration }
        if let motivationType = motivationType { body["motivationType"] = motivationType }
        if let trainingLevel = trainingLevel { body["trainingLevel"] = trainingLevel }
        if let specificSport = specificSport { body["specificSport"] = specificSport }
        if let selectedGymId = selectedGymId { body["selectedGymId"] = selectedGymId }
    ...
    func selectGym(gymId: String) async throws {
        guard let token = authToken else {
            throw APIError.unauthorized
        }
        
        let url = URL(string: "\(baseURL)/api/gyms/\(gymId)/select")!
        var request = URLRequest(url: url)
        request.httpMethod = "PATCH"
        request.setValue("Bearer \(token)", forHTTPHeaderField: "Authorization")
    ...
    func resetUserGyms() async throws {
        let url = URL(string: "\(baseURL)/api/gyms/reset")!
        var request = URLRequest(url: url)
        request.httpMethod = "POST"
        
        if let token = authToken {
            request.setValue("Bearer \(token)", forHTTPHeaderField: "Authorization")
            print("[API] 🔑 Using auth token for reset gyms request")
        } else {
            print("[API] ⚠️ No auth token available, trying without (dev mode)")
        }
    ...
    func fetchGymEquipment(gymId: String) async throws -> [UserEquipmentResponse] {
        guard let token = authToken else {
            throw APIError.unauthorized
        }
        
        let url = URL(string: "\(baseURL)/api/gyms/\(gymId)/equipment")!
        var request = URLRequest(url: url)
        request.httpMethod = "GET"
        request.setValue("Bearer \(token)", forHTTPHeaderField: "Authorization")
    ...
    func createGym(
        ...
        equipmentIds: [String]? = nil,
        isPublic: Bool = false,
        isVerified: Bool? = nil
    ) async throws -> GymResponse {
        guard let token = authToken else {
            throw APIError.unauthorized
        }
    ...
    func deleteEquipment(id: String) async throws {
        ...
        let url = URL(string: "\(baseURL)/api/equipment/\(id)")!
        var request = URLRequest(url: url)
        request.httpMethod = "DELETE"
        request.setValue("Bearer \(token)", forHTTPHeaderField: "Authorization")
