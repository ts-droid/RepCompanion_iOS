...
class SyncService: ObservableObject {
    ...
    func syncGymsAndEquipment(userId: String, modelContext: ModelContext) async throws {
        ...
        for existingGym in existingGyms {
            if !serverGymIds.contains(existingGym.id) && existingGym.id != selectedGymId {
        ...
        // Fetch equipment from API (user-owned equipment)
        ...
        // Fetch equipment directly per gym — needed for verified/public gyms
        // whose equipment is NOT owned by the current user and therefore not
        // returned by /api/equipment.
        print("[SYNC] 🔄 Fetching per-gym equipment from server...")
        let allGymsCurrent = (try? modelContext.fetch(FetchDescriptor<Gym>(predicate: #Predicate { $0.userId == userId }))) ?? []
        for gym in allGymsCurrent {
            do {
                let gymEquipmentData = try await apiService.fetchGymEquipment(gymId: gym.id)
                let gymEquipNames = gymEquipmentData.filter { $0.available }.map { $0.equipmentName }

                // Upsert UserEquipment records for this gym
                let currentGymId = gym.id
                let gymEquipDescriptor = FetchDescriptor<UserEquipment>(
                    predicate: #Predicate { $0.gymId == currentGymId && $0.userId == userId }
                )
                let localGymEquip = (try? modelContext.fetch(gymEquipDescriptor)) ?? []
                let localGymEquipNames = Set(localGymEquip.map { $0.equipmentName })

                for equipData in gymEquipmentData where equipData.available {
                    if !localGymEquipNames.contains(equipData.equipmentName) {
                        let eq = UserEquipment(
                            id: equipData.id,
                            userId: userId,
                            gymId: gym.id,
                            equipmentType: equipData.equipmentType,
                ...
                if !gymEquipNames.isEmpty {
                    gym.equipmentIds = gymEquipNames
                    print("[SYNC] ✅ Gym '\(gym.name)': \(gymEquipNames.count) equipment items synced")
                }
            } catch {
                print("[SYNC] ⚠️ Could not fetch equipment for gym '\(gym.name)': \(error)")
            }
        }

        try modelContext.save()

        // RECONCILE: Update Gym.equipmentIds from UserEquipment (catch-all for any remaining gaps)
    ...
    func migrateActiveGymIfNeeded(userId: String, modelContext: ModelContext) async {
        do {
            try await syncGymsAndEquipment(userId: userId, modelContext: modelContext)
        } catch {
            print("[SyncService] ⚠️ Gym sync failed during migration: \(error)")
        }
        ...
        guard !userGyms.isEmpty else { return }
        
        let gymIds = Set(userGyms.map { $0.id })
        let currentSelected = profile.selectedGymId
        let selectedExists = currentSelected != nil && gymIds.contains(currentSelected!)
        
        if !selectedExists {
            let sortedGyms = userGyms.sorted {
            ...
            let newSelectedId = sortedGyms.first?.id
            profile.selectedGymId = newSelectedId
            
            for gym in userGyms {
                gym.isSelected = (gym.id == newSelectedId)
            }
            
            try? modelContext.save()
            
            if let newSelectedId {
                try? await APIService.shared.selectGym(gymId: newSelectedId)
                print("[SyncService] ✅ Migrated active gym to \(newSelectedId)")
            }
        }
    }

    func backfillVerifiedGymEquipmentIfNeeded(userId: String, modelContext: ModelContext) async {
        let gymsDescriptor = FetchDescriptor<Gym>(
            predicate: #Predicate { $0.userId == userId }
        )
        let userGyms = (try? modelContext.fetch(gymsDescriptor)) ?? []
        ...
        for gym in userGyms {
            guard gym.isVerified, gym.equipmentIds.isEmpty else { continue }
            guard let lat = gym.latitude, let lon = gym.longitude else { continue }
            
            do {
                let nearby = try await APIService.shared.fetchNearbyGyms(
                    lat: lat,
                    lng: lon,
                    radiusKm: 2
                )
                
                let match = nearby.first {
                    $0.name.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
                        == gym.name.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
                }
                
                guard let matchedGym = match else { continue }
                
                let equipment = try await APIService.shared.fetchGymEquipment(gymId: matchedGym.id)
                let equipmentNames = equipment
                    .filter { $0.available }
                    .map { $0.equipmentName }
                
                guard !equipmentNames.isEmpty else { continue }
                ...
                for equipmentName in equipmentNames {
                    ...
                    let localEquipment = UserEquipment(
                        id: UUID().uuidString,
                        userId: userId,
                        gymId: gym.id,
                        equipmentType: "gym",
                    ...
                    modelContext.insert(localEquipment)
                }
                
                gym.equipmentIds = equipmentNames
                try? modelContext.save()
                print("[SyncService] ✅ Backfilled equipment for verified gym: \(gym.name)")
            } catch {
                print("[SyncService] ⚠️ Failed to backfill equipment for gym \(gym.name): \(error)")
            }
        }
    }
    ...
    private func updateProfile(_ profile: UserProfile, with data: UserProfileResponse) {
        ...
        if let serverGymId = data.selectedGymId {
            profile.selectedGymId = serverGymId
        }
        
        if previousGymId != profile.selectedGymId {
            print("[SyncService] 🏋️ Updated selectedGymId: \(previousGymId ?? "nil") → \(profile.selectedGymId ?? "nil")")
        }
    }
