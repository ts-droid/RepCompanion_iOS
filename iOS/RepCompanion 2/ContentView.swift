...
struct ContentView: View {
    ...
    @AppStorage("didMigrateActiveGym") private var didMigrateActiveGym = false
    ...
    var body: some View {
        ...
        .task(id: authService.currentUserId) {
            ...
            if !didMigrateActiveGym {
                await SyncService.shared.migrateActiveGymIfNeeded(userId: userId, modelContext: modelContext)
                await SyncService.shared.backfillVerifiedGymEquipmentIfNeeded(userId: userId, modelContext: modelContext)
                didMigrateActiveGym = true
            }
        }
    }
